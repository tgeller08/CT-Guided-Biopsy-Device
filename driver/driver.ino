...
char* convert_int16_to_str_o(int16_t i) { // converts int16 to string. Moreover, resulting strings will have the same length in the debug monitor.
...
void loop()
{
  ...
  // print out data
  Serial.print("aX = "); Serial.print(convert_int16_to_str_o(accelerometer_x));
  Serial.print(" | aY = "); Serial.print(convert_int16_to_str_o(accelerometer_y));
  Serial.print(" | aZ = "); Serial.print(convert_int16_to_str_o(accelerometer_z));
  // the following equation was taken from the documentation [MPU-6000/MPU-6050 Register Map and Description, p.30]
  Serial.print(" | orig_tmp = "); Serial.print(temperature);
  Serial.print(" | tmp = "); Serial.print(temperature/340.00+36.53);
  Serial.print(" | gX = "); Serial.print(convert_int16_to_str_o(gyro_x));
  Serial.print(" | gY = "); Serial.print(convert_int16_to_str_o(gyro_y));
  Serial.print(" | gZ = "); Serial.print(convert_int16_to_str_o(gyro_z));
  Serial.println();
...
//  bleuart.write(newline);
  ...
  char X_ANGLE[20];
  convert_int16_to_str(x, X_ANGLE);
  char X_ANGLE_LABEL[14] = {'X',' ','A','N','G','L','E',':',' '};
  bleuart.write(X_ANGLE_LABEL);
  bleuart.write(X_ANGLE);
  bleuart.write(newline);
  
  char Y_ANGLE[20];
  convert_int16_to_str(y, Y_ANGLE);
  char Y_ANGLE_LABEL[14] = {'Y',' ','A','N','G','L','E',':',' '};
  bleuart.write(Y_ANGLE_LABEL);
  bleuart.write(Y_ANGLE);
  bleuart.write(newline);
  
  char Z_ANGLE[20];
  convert_int16_to_str(z, Z_ANGLE);
  char Z_ANGLE_LABEL[14] = {'Z',' ','A','N','G','L','E',':',' '};
  bleuart.write(Z_ANGLE_LABEL);
  bleuart.write(Z_ANGLE);
  bleuart.write(newline);
